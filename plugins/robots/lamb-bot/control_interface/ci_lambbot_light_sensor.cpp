#include "ci_lambbot_light_sensor.h"
...
namespace argos {
   ...
   CCI_LambBotLightSensor::CCI_LambBotLightSensor() :
      m_tReadings(24) {
   ...
   const CCI_LambBotLightSensor::TReadings& CCI_LambBotLightSensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_LambBotLightSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LambBotLightSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LambBotLightSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LambBotLightSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
