#include "ci_lambbot_proximity_sensor.h"
...
namespace argos {
   ...
   CCI_LambBotProximitySensor::CCI_LambBotProximitySensor() :
      m_tReadings(24) {
   ...
   const CCI_LambBotProximitySensor::TReadings& CCI_LambBotProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_LambBotProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LambBotProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LambBotProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LambBotProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
