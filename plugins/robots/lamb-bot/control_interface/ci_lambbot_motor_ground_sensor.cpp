#include "ci_lambbot_motor_ground_sensor.h"
...
namespace argos {
   ...
   CCI_LambBotMotorGroundSensor::CCI_LambBotMotorGroundSensor() :
      m_tReadings(4) {
   ...
   const CCI_LambBotMotorGroundSensor::TReadings& CCI_LambBotMotorGroundSensor::GetReadings() const {
     return m_tReadings;
   }
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LambBotMotorGroundSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LambBotMotorGroundSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LambBotMotorGroundSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LambBotMotorGroundSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
