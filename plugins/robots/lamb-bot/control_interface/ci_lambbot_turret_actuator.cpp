#include "ci_lambbot_turret_actuator.h"
...
namespace argos {
  ...
  const CRange<SInt32> CCI_LambBotTurretActuator::SPEED_RANGE(-4,4);
  const CRange<Real>   CCI_LambBotTurretActuator::NORMALIZED_SPEED_RANGE(-1.0,1.0);
  ...
  int LuaTurretSetRotation(lua_State* pt_lua_state) {
    ...
    /* Perform action */
    CLuaUtility::GetDeviceInstance<CCI_LambBotTurretActuator>(pt_lua_state, "turret")->
  ...
  void CCI_LambBotTurretActuator::SetActiveWithRotation(const CRadians& c_angle) {
    SetPositionControlMode();
    SetRotation(c_angle);
  }
  ...
  void CCI_LambBotTurretActuator::SetSpeedControlMode() {
  ...
  void CCI_LambBotTurretActuator::SetPositionControlMode() {
  ...
  void CCI_LambBotTurretActuator::SetPassiveMode() {
...
#ifdef ARGOS_WITH_LUA
  void CCI_LambBotTurretActuator::CreateLuaState(lua_State* pt_lua_state) {
