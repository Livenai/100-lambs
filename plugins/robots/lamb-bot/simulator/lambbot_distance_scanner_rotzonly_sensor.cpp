/**
 * @file <argos3/plugins/robots/lamb-bot/simulator/lambbot_distance_scanner_rotzonly_sensor.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "lambbot_distance_scanner_rotzonly_sensor.h"
...
namespace argos {
   ...
   static const Real SHORT_RANGE_RAY_START    = LAMBBOT_RADIUS;
   static const Real SHORT_RANGE_RAY_END      = LAMBBOT_RADIUS + 0.26f;

   static const Real LONG_RANGE_MIN_DISTANCE  = 0.12f;
   static const Real LONG_RANGE_RAY_START     = LAMBBOT_RADIUS;
   static const Real LONG_RANGE_RAY_END       = LAMBBOT_RADIUS + 1.42f;
   ...
   CLambBotDistanceScannerRotZOnlySensor::CLambBotDistanceScannerRotZOnlySensor() :
      m_pcRNG(NULL),
      m_bAddNoise(false),
   ...
   void CLambBotDistanceScannerRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_LambBotDistanceScannerSensor::Init(t_tree);
         /* Show rays? */
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in lamb-bot distance scanner rot_z_only sensor.", ex);
      }
   }
   ...
   void CLambBotDistanceScannerRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      m_pcEmbodiedEntity = &(c_entity.GetComponent<CEmbodiedEntity>("body"));
      m_pcControllableEntity = &(c_entity.GetComponent<CControllableEntity>("controller"));
      m_pcDistScanEntity = &(c_entity.GetComponent<CLambBotDistanceScannerEquippedEntity>("distance_scanner"));
   ...
   void CLambBotDistanceScannerRotZOnlySensor::Update() {
      /* Clear the maps */
      m_tReadingsMap.clear();
      m_tShortReadingsMap.clear();
      m_tLongReadingsMap.clear();
      /* Perform calculations only if the sensor is on */
      if(m_pcDistScanEntity->GetMode() != CLambBotDistanceScannerEquippedEntity::MODE_OFF) {
         /* Update the readings wrt to device mode */
         if(m_pcDistScanEntity->GetMode() == CLambBotDistanceScannerEquippedEntity::MODE_POSITION_CONTROL) {
   ...
   void CLambBotDistanceScannerRotZOnlySensor::Reset() {
      /* Clear the maps */
      m_tReadingsMap.clear();
      m_tShortReadingsMap.clear();
      m_tLongReadingsMap.clear();
   ...
   void CLambBotDistanceScannerRotZOnlySensor::UpdateNotRotating() {
      /* Short range [0] */
...
#define ADD_READINGS(RAYS,MAP,MINDIST)          \
   ...
   void CLambBotDistanceScannerRotZOnlySensor::UpdateRotating() {
   ...
   Real CLambBotDistanceScannerRotZOnlySensor::CalculateReadingForRay(const CRay3& c_ray,
                                                                      Real f_min_distance) {
...
#define CALCULATE_LONG_RANGE_RAY(ANGLE,INDEX)               \
   ...
   void CLambBotDistanceScannerRotZOnlySensor::CalculateRaysNotRotating() {
      /* We make the assumption that the lamb-bot is rotated only around Z */
      /* Get the lamb-bot orientation */
      ...
      /* The short range sensors are oriented along the lamb-bot local X */
      m_cDirection = CVector3::X;
      CALCULATE_SHORT_RANGE_RAY(cAbsoluteOrientation, 0);
      /* The short range sensors are oriented along the lamb-bot local Y */
   ...
   void CLambBotDistanceScannerRotZOnlySensor::CalculateRaysRotating() {
      /* We make the assumption that the lamb-bot is rotated only around Z */
      /* Get the lamb-bot orientation */
      CRadians cTmp1, cTmp2, cOrientationZ;
      m_pcEmbodiedEntity->GetOriginAnchor().Orientation.ToEulerAngles(cOrientationZ, cTmp1, cTmp2);
      ...
      /* The long range sensors are oriented along the lamb-bot local Y */
      m_cDirection = CVector3::Y;
      CALCULATE_LONG_RANGE_RAY(cAbsoluteOrientation, 0);
   ...
   REGISTER_SENSOR(CLambBotDistanceScannerRotZOnlySensor,
                   "lambbot_distance_scanner", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The lamb-bot distance scanner sensor (optimized for 2D).",
                   "This sensor accesses the lamb-bot distance scanner sensor. For a complete\n"
                   ...
                   "the lamb-bot is always parallel to the XY plane, i.e., it rotates only around\n"
                   "the Z axis. This implementation is faster than a 3D one and should be used\n"
                   "only when the assumption about the lamb-bot rotation holds.\n\n"
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <lambbot_distance_scanner implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
