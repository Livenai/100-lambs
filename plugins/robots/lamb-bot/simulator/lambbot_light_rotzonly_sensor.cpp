/**
 * @file <argos3/plugins/robots/lamb-bot/simulator/lambbot_light_rotzonly_sensor.cpp>
 *
...
#include "lambbot_light_rotzonly_sensor.h"
...
namespace argos {
   ...
   CLambBotLightRotZOnlySensor::CLambBotLightRotZOnlySensor() :
      m_bEnabled(true),
   ...
   void CLambBotLightRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the lamb-bot light default sensor", ex);
      }
   }
   ...
   void CLambBotLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CLambBotLightRotZOnlySensor::Update() {
      if (!m_bEnabled) {
        return;
      }
      /* Erase readings */
      ...
      /* Buffer for the angle of the light wrt to the lamb-bot */
      CRadians cAngleLightWrtLambbot;
       ...
       */
         for(CSpace::TMapPerType::iterator it = mapLights.begin();
            ...
            if(cLight.GetIntensity() > 0.0f) {
               ...
               /* Check occlusion between the lamb-bot and the light */
               if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
                  ...
                  /* Get the distance between the light and the lamb-bot */
                  ...
                  /* Get the angle wrt to lamb-bot rotation */
                  cAngleLightWrtLambbot = cRobotToLight.GetZAngle();
                  cAngleLightWrtLambbot -= cOrientationZ;
                  /*
                   * Find closest sensor index to point at which ray hits lambbot body
                   * Rotate whole body by half a sensor spacing (corresponding to placement of first sensor)
                   * Division says how many sensor spacings there are between first sensor and point at which ray hits lambbot body
                   * Increase magnitude of result of division to ensure correct rounding
                   */
                  Real fIdx = (cAngleLightWrtLambbot - SENSOR_HALF_SPACING) / SENSOR_SPACING;
                  ...
                  for(SInt32 nIndexOffset = -6; nIndexOffset < 7; ++nIndexOffset) {
                     ...
                     CRadians cAngularDistanceFromOptimalLightReceptionPoint = Abs((cAngleLightWrtLambbot - m_tReadings[unIdx].Angle).SignedNormalize());
                     /*
   ...
   void CLambBotLightRotZOnlySensor::Reset() {
   ...
   void CLambBotLightRotZOnlySensor::Enable() {
     m_bEnabled = true;
   }
   ...
   void CLambBotLightRotZOnlySensor::Disable() {
     m_bEnabled = false;
   }
   ...
   REGISTER_SENSOR(CLambBotLightRotZOnlySensor,
                   "lambbot_light", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The lamb-bot light sensor (optimized for 2D).",
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <lambbot_light implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
