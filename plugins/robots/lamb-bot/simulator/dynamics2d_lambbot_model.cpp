#include "dynamics2d_lambbot_model.h"
...
namespace argos {
   ...
   static const Real LAMBBOT_MAX_FORCE                = 15.f;
   static const Real LAMBBOT_MAX_TORQUE               = 150.f;
   ...
   CDynamics2DLambBotModel::CDynamics2DLambBotModel(CDynamics2DEngine& c_engine,
                                                    CLambBotEntity& c_entity) :
      CDynamics2DMultiBodyObjectModel(c_engine, c_entity),
      m_cLambBotEntity(c_entity),
      m_cWheeledEntity(m_cLambBotEntity.GetWheeledEntity()),
      m_cGripperEntity(c_entity.GetGripperEquippedEntity()),
      m_cDiffSteering(c_engine,
                      LAMBBOT_MAX_FORCE,
                      LAMBBOT_MAX_TORQUE,
                      LAMBBOT_INTERWHEEL_DISTANCE,
                      c_entity.GetConfigurationNode()),
      m_pcGripper(NULL),
      m_pcGrippable(NULL),
      m_fMass(1.6f),
      m_fCurrentWheelVelocity(m_cWheeledEntity.GetWheelVelocities()),
      m_unLastTurretMode(m_cLambBotEntity.GetTurretEntity().GetMode()) {
      RegisterAnchorMethod<CDynamics2DLambBotModel>(
         GetEmbodiedEntity().GetOriginAnchor(),
         &CDynamics2DLambBotModel::UpdateOriginAnchor);
      RegisterAnchorMethod<CDynamics2DLambBotModel>(
         GetEmbodiedEntity().GetAnchor("turret"),
         &CDynamics2DLambBotModel::UpdateTurretAnchor);
      RegisterAnchorMethod<CDynamics2DLambBotModel>(
         GetEmbodiedEntity().GetAnchor("perspective_camera"),
         &CDynamics2DLambBotModel::UpdatePerspectiveCameraAnchor);
      ...
      AddBody(m_ptActualBaseBody, cpvzero, 0, LAMBBOT_HEIGHT);
      ...
      m_ptBaseGripperAngularMotion->maxForce = LAMBBOT_MAX_TORQUE; /* limit the dragging torque */
      /* Add the gripper body so that the default methods work as expected */
      AddBody(m_ptActualGripperBody, cpvzero, 0, LAMBBOT_HEIGHT);
   ...
   CDynamics2DLambBotModel::~CDynamics2DLambBotModel() {
      delete m_pcGripper;
   ...
   void CDynamics2DLambBotModel::MoveTo(const CVector3& c_position,
   ...
   void CDynamics2DLambBotModel::Reset() {
   ...
   void CDynamics2DLambBotModel::CalculateBoundingBox() {
      ...
      GetBoundingBox().MaxCorner.SetZ(GetDynamics2DEngine().GetElevation() + LAMBBOT_HEIGHT);
   }

   /****************************************/
   /****************************************/

   void CDynamics2DLambBotModel::UpdateFromEntityStatus() {
      /* Do we want to move? */
      if((m_fCurrentWheelVelocity[LAMBBOT_LEFT_WHEEL] != 0.0f) ||
         (m_fCurrentWheelVelocity[LAMBBOT_RIGHT_WHEEL] != 0.0f)) {
         m_cDiffSteering.SetWheelVelocity(m_fCurrentWheelVelocity[LAMBBOT_LEFT_WHEEL],
                                          m_fCurrentWheelVelocity[LAMBBOT_RIGHT_WHEEL]);
      }
      else {
      ...
      if(m_cLambBotEntity.GetTurretEntity().GetMode() != m_unLastTurretMode) {
         /* Enable or disable the anchor */
         if(m_cLambBotEntity.GetTurretEntity().GetMode() != MODE_OFF) {
         ...
         switch(m_unLastTurretMode) {
            case MODE_OFF:
            case MODE_PASSIVE:
               switch(m_cLambBotEntity.GetTurretEntity().GetMode()) {
                  case MODE_POSITION_CONTROL:
                  case MODE_SPEED_CONTROL:
      ...
      switch(m_unLastTurretMode) {
         ...
         case MODE_POSITION_CONTROL: {
            Real fCurRotErr = NormalizedDifference(
               m_cLambBotEntity.GetTurretEntity().GetDesiredRotation(),
         ...
         case MODE_SPEED_CONTROL:
            m_ptControlGripperBody->w =
               m_cDiffSteering.GetAngularVelocity() +
               m_cLambBotEntity.GetTurretEntity().GetDesiredRotationSpeed();
            break;
         case MODE_OFF:
         case MODE_PASSIVE:
   ...
   void CDynamics2DLambBotModel::TurretPassiveToActive() {
      ...
      m_ptGripperControlAngularMotion->maxForce = LAMBBOT_MAX_TORQUE; /* limit the dragging torque */
   }

   /****************************************/
   /****************************************/

   void CDynamics2DLambBotModel::TurretActiveToPassive() {
   ...
   void CDynamics2DLambBotModel::UpdateOriginAnchor(SAnchor& s_anchor) {
   ...
   void CDynamics2DLambBotModel::UpdateTurretAnchor(SAnchor& s_anchor) {
   ...
   void CDynamics2DLambBotModel::UpdatePerspectiveCameraAnchor(SAnchor& s_anchor) {
   ...
   REGISTER_STANDARD_DYNAMICS2D_OPERATIONS_ON_ENTITY(CLambBotEntity, CDynamics2DLambBotModel);
