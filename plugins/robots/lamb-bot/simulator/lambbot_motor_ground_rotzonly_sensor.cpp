/**
 * @file <argos3/plugins/robots/lamb-bot/simulator/lambbot_motor_ground_rotzonly_sensor.cpp>
 *
...
#include "lambbot_motor_ground_rotzonly_sensor.h"
...
namespace argos {
   ...
   CLambBotMotorGroundRotZOnlySensor::CLambBotMotorGroundRotZOnlySensor() :
   ...
   void CLambBotMotorGroundRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
   ...
   void CLambBotMotorGroundRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_LambBotMotorGroundSensor::Init(t_tree);
         /* Parse noise level */
         Real fNoiseLevel = 0.0f;
         GetNodeAttributeOrDefault(t_tree, "noise_level", fNoiseLevel, fNoiseLevel);
         if(fNoiseLevel < 0.0f) {
            THROW_ARGOSEXCEPTION("Can't specify a negative value for the noise level of the lamb-bot ground sensor");
         }
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in lamb-bot rotzonly ground sensor", ex);
      }
   }
   ...
   void CLambBotMotorGroundRotZOnlySensor::Update() {
      /*
   ...
   void CLambBotMotorGroundRotZOnlySensor::Reset() {
   ...
   REGISTER_SENSOR(CLambBotMotorGroundRotZOnlySensor,
                   "lambbot_motor_ground", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The lamb-bot motor ground sensor.",
                   "This sensor accesses the lamb-bot motor ground sensor. For a complete description\n"
                   "of its usage, refer to the ci_lambbot_motor_ground_sensor.h interface. For the XML\n"
