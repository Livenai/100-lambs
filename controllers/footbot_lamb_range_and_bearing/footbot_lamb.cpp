...
void CFootBotLamb::SendMessage(){
    if(GetId() == "lamb_0"){
        // to_string(mess_count);
        UInt8 *data_b = new UInt8[10];
        data_b[0] = mess_count;
        CByteArray mess_data(data_b, 10);
...
void CFootBotLamb::PollMessages(){
    if(GetId() != "lamb_0" ){
        CCI_RangeAndBearingSensor::TReadings packets = m_pcRBSens->GetReadings();
        if (packets.size() != 0){
            LOG<<GetId()<<" recibe: "<<packets[0].Data[0]<<std::endl;
        }
    }
}
